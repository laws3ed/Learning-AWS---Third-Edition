...
print("Spark parquet gzipped processing start time: ", datetime.now())

start = time.time()

spark = SparkSession.builder.master("local[*]").config("spark.sql.parquet.compression.codec", "gzip").config("spark.driver.host", "localhost").config("spark.sql.shuffle.partitions", "3000").config("spark.sql.broadcastTimeout", "36000").appName('PySpark Read CSV').config("spark.driver.memory", "230g").config("spark.network.timeout", "10000000").config("spark.executor.memory", "230g").config("spark.driver.maxResultSize", "230g").getOrCreate()
spark.sparkContext.setLogLevel("ERROR")

df1 = spark.read.parquet("/work/data/parquet_gzipped_dir_1")
df1=df1.sort("event_time1")
df2 = spark.read.parquet("/work/data/parquet_gzipped_dir_2")
...
df_result2=scaledData2.withColumn("feature21", multiplyColumns(col("C21_scaled"), col("C22_scaled")))
...
#print(column_names)
selected = [s for s in column_names if ('_vec' not in s) and ('__null_dask_index__' not in s)]
print(selected)
...
df_final.write.mode("overwrite").parquet("/work/data/spark_output_parquet_gzipped")
end = time.time()
# print the total processing time taken
print("Total time: {:.3f}".format(end-start)," sec")
print("Spark parquet gzipped processing end time: ", datetime.now())
